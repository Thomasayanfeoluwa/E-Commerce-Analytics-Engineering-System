...
def repeat_purchase_rate():
    return run_sql("""
        ...
        SELECT
            COUNT(*) FILTER (WHERE n_orders > 1)::float / COUNT(*) AS repeat_rate
        FROM order_counts;
    """)

def customer_lifetime_value_proxy():
    return run_sql("""
        SELECT user_id, SUM(total) AS total_spent, COUNT(*) AS orders
        FROM orders
        WHERE status NOT IN ('cancelled')
        GROUP BY user_id
...
def return_rate():
    return run_sql("""
        SELECT
            COUNT(*) FILTER (WHERE status = 'returned')::float / COUNT(*) AS return_rate
        FROM orders;
    """)
...
def revenue_by_country():
    return run_sql("""
        SELECT a.country_code, SUM(o.total) AS revenue
        FROM orders o
...
def retention():
    return run_sql("""
        ...
        activity AS (
            SELECT o.user_id, f.cohort_month, DATE_TRUNC('month', o.created_at) AS order_month
            FROM orders o
...
def first_to_second_purchase_interval():
    return run_sql("""
        ...
        SELECT AVG(r2.created_at - r1.created_at) AS avg_interval
        FROM ranked r1
